app.controller('ethereumController', function($scope, $http, $location, $route, $log, $q) {
	...
	$http({
		method : 'GET',
		url : '/api/ethereum/lastvalues/' + cryptocurrencyId
...
//		$scope.colors = ['rgb(65, 65, 65)', 'rgb(150, 65, 65)', 'rgb(65, 150, 65)', 'rgb(65, 65, 150)', 'rgb(65, 150, 150)'];
		...
		chartGeneratorForEth($http, $scope, $q, '/api/ethereum/lastDay/', 'ETH/USD ( Ethereum )', 'minute', 'daily');
		chartGeneratorForEth($http, $scope, $q, '/api/ethereum/lastWeek/',  'ETH/USD ( Ethereum )', 'day', 'weekly');
		chartGeneratorForEth($http, $scope, $q, '/api/ethereum/lastMonth/', 'ETH/USD ( Ethereum )', 'day', 'monthly');
		chartGeneratorForEth($http, $scope, $q, '/api/ethereum/lastYear/',  'ETH/USD ( Ethereum )', 'month', 'yearly');
	});
});

function chartGeneratorForEth($http, $scope, $q, urlBase, chartTitle, unit, elementId) {
	var actions = [];
	for(var i = 0; i < $scope.providers.length; i++) {
